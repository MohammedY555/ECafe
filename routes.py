...
def add_menu_route():
    ...
    if not all([title, title_id, description, description_id, status, status_time, price]):
        return jsonify({'error': 'Missing required fields for menu item.'}), 400

    try:
        add_menu(Session, title, title_id, description, description_id, status, status_time, price)
...
def add_personal_route():
    ...
    if not all([job_title, job_title_id, salary, first_name, last_name, age, status]):
        return jsonify({'error': 'Missing required fields for personal data.'}), 400

    try:
        add_personal(Session, job_title, job_title_id, salary, first_name, last_name, age, status)
...
def update_table_status_route(table_status_id):
    ...
    try:
        is_updated = update_table_status(Session, table_status_id, new_status_name)
        print(is_updated)
        # if is_updated
